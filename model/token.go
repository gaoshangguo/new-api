...
type Token struct {
	...
	DailyQuota            int64          `json:"daily_quota" gorm:"bigint"`       // 日预算（0=不限）
	MonthlyQuota          int64          `json:"monthly_quota" gorm:"bigint"`     // 月预算（0=不限）
	ChannelLimits         *string        `json:"channel_limits" gorm:"type:text"` // Key 级渠道白名单 JSON {"channels":[id...]}
	MaxConcurrentRequests int            `json:"max_concurrent_requests"`         // 并发上限（0=不限）
	RateLimitRPM          int            `json:"rate_limit_rpm"`                  // 每分钟请求数（0=不限）
	RateLimitTPM          int64          `json:"rate_limit_tpm" gorm:"bigint"`    // 每分钟 token 预估（0=不限）
...
func (token *Token) Update() (err error) {
	...
	err = DB.Model(token).Select("name", "status", "expired_time", "remain_quota", "unlimited_quota",
		...
		"daily_quota", "monthly_quota", "channel_limits", "max_concurrent_requests", "rate_limit_rpm", "rate_limit_tpm").Updates(token).Error
	return err
}
