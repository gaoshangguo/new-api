...
export function getApiKeyFormSchema(t: TFunction, maxAutoGroups = 5) {
  ...
  return z
    .object({
      ...
      daily_quota: z.coerce.number().int().min(0).default(0),
      monthly_quota: z.coerce.number().int().min(0).default(0),
      max_concurrent_requests: z.coerce.number().int().min(0).default(0),
      rate_limit_rpm: z.coerce.number().int().min(0).default(0),
      rate_limit_tpm: z.coerce.number().int().min(0).default(0),
      channel_limits: z.string().optional(),
...
export function transformFormDataToPayload(
  ...
  return {
    ...
    max_concurrent_requests: Number(data.max_concurrent_requests) || 0,
    rate_limit_rpm: Number(data.rate_limit_rpm) || 0,
    rate_limit_tpm: Number(data.rate_limit_tpm) || 0,
    channel_limits: (data.channel_limits || '').trim() || '',
    group: data.group || '',
    auto_groups:
...
export function transformApiKeyToFormDefaults(
  ...
  return {
    ...
    daily_quota: Number(apiKey.daily_quota) || 0,
    monthly_quota: Number(apiKey.monthly_quota) || 0,
    max_concurrent_requests: Number(apiKey.max_concurrent_requests) || 0,
    rate_limit_rpm: Number(apiKey.rate_limit_rpm) || 0,
    rate_limit_tpm: Number(apiKey.rate_limit_tpm) || 0,
