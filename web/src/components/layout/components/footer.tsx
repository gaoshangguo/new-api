  ...
  if (isExternal) {
    return (
      <a
        ...
        className='text-footer-muted hover:text-footer-foreground text-sm transition-colors duration-200'
      >
        {label}
      </a>
    )
      ...
      {items.map((item, index) => (
        <Fragment key={item.key}>
          {(props.leadingSeparator || index > 0) && (
            <span aria-hidden='true' className='text-footer-muted/40'>
              ·
            </span>
          )}
          <Link
            to={item.href}
            className='text-footer-muted hover:text-footer-foreground transition-colors duration-200'
          >
            {item.label}
          </Link>
        </Fragment>
...
export function Footer(props: FooterProps) {
  ...
  if (footerHtml) {
    return (
      <footer
        className={cn(
          'bg-footer text-footer-foreground border-footer-border relative z-10 border-t',
          props.className
        )}
      >
        <div className='mx-auto w-full max-w-6xl px-6 py-5'>
          <div className='bg-footer-foreground/5 border-footer-border flex flex-col items-center justify-between gap-4 rounded-2xl border px-4 py-4 backdrop-blur-sm sm:flex-row sm:px-5'>
            <div
              className='custom-footer text-footer-muted min-w-0 text-center text-sm sm:text-left'
      ...
      <div className='mx-auto max-w-6xl px-6 py-12 md:py-16'>
        <div className='flex flex-col justify-between gap-10 md:flex-row md:gap-16'>
          ...
          <div className='shrink-0'>
            ...
            <p className='text-footer-muted mt-3 max-w-[200px] text-xs leading-relaxed'>
          ...
          {isDemoSiteMode && (
            <div className='grid grid-cols-3 gap-8 md:gap-16'>
              {displayColumns.map((column, index) => (
                <div key={index}>
                  <p className='text-footer-muted/70 mb-3 text-xs font-medium tracking-wider uppercase'>
                    {t(column.title)}
                  </p>
