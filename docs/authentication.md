...
开放注册仍会受 Critical IP 限流保护，但分布式 IP 多账号攻击不能仅靠 IP 限流阻止。公网开放注册的部署应至少启用图形验证码或 Turnstile，并建议同时启用邮箱验证；更强的设备或多维风控需作为独立安全项目设计。

## 注册图形验证码

`CaptchaEnabled`（管理后台「Bot Protection - Enable image captcha」）为自托管的图片验证码开关，与 `TurnstileCheckEnabled` 相互独立，可单独或同时启用；启用后 `POST /api/user/register` 必须携带 `captcha_id` 与 `captcha_code`。

`GET /api/captcha` 下发一次性挑战：4 位去混淆字符（无 `0/O`、`1/I/L`）、PNG data URL、5 分钟有效。校验发生在注册逻辑最前面且在触碰数据库之前，无论校验成功与否都会立即作废该挑战，因此同一张图片不能重复提交或反复猜测。挑战默认存放于进程内 LRU（上限 20000 条）；启用 Redis 时写入 Redis，多节点部署下各节点共享同一份挑战。
